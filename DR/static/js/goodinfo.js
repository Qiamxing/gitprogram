...
$(function(){
	...
	// 	}
				...
				for(var i=0;i<json.length;i++){
					...
					for(var j=0;j<arr.length;j++){
						...
						var obj = arr[j];//第二次遍历每个数组中的对象
							...
							console.log(obj.id);
								// 创建新的节点
								smallImg = $('<img/>').addClass('smallImg').attr('src',obj.src);
								bigImg = $('<img/>').addClass('bigImg').attr('src',obj.src);
								...
								console.log(obj.id);
								// console.log('==================='+obj.img[0].src);
								// 左侧四张小图
								for(var i=0;i<obj.img.length;i++){
									var liNode = $("<li/>");
									var imgNode = $('<img/>').attr('src',obj.img[i].src);
		...
		$('.bc_left li').on('mouseenter',function(){
			...
			console.log($('.bc_left li img').eq($(this).index()).attr('src'));
			$('.bc_center li img').attr({
				'src':$('.bc_left li img').eq($(this).index()).attr('src')
			});
			$(this).addClass('box').siblings().removeClass('box');
			$(this).find('img').css('border','none').siblings().css('border','1px solid #dadada');
			$('.bigArea img').attr({
				'src':$('.bc_left li img').eq($(this).index()).attr('src')
			});
		})
		...
		$('.bc_center i').on('click',function(){
			...
			$('.bc_center li img').attr({
				'src':$('.bc_left li img').eq(img_index).attr('src')
			});
			$('.bigArea img').attr({
				'src':$('.bc_left li img').eq(img_index).attr('src')
			});
		...
		// $('.bc_left li img').on({
		// 	'mouseenter':function(){
		// 		$('.bc_center li img').css('src',$('.bc_left li img').src);
		// 	}
		// })
