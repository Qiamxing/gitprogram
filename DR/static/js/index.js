	...
	$('.drstore-tab li').on('mouseenter',function(){
		...
		$.getJSON("json/index.json",function(json){
			...
			var timer = setInterval(move,2000);
				...
				function move(){
				if(i<=3){
					$('.dright-img li').eq(i).stop().animate({opacity:1}).siblings().animate({opacity:0});
					i++;
					// console.log(i)
				}else if(i>3){
					i=0;
			...
			for(var i=0;i<json[7].length;i++){
				$('.dright-img li').eq(i).append($('<img/>').attr('src',json[7][i].src));
				$('.dright-img li').eq(i).append($('<a/>').text(json[7][i].title));
			}

			$('.dright-img li').hover(function(){
				clearInterval(timer);
			...
			for(var i=0;i<json[6][0].length;i++){
				$('img.t1').eq(i).attr('src',json[6][0][i].src);
			}
			for(var i=0;i<json[6][1].length;i++){
				$('img.t2').eq(i).attr('src',json[6][1][i].src);
			}
			for(var i=0;i<json[6][2].length;i++){
				$('img.t3').eq(i).attr('src',json[6][2][i].src);
			}
			for(var i=0;i<json[6][3].length;i++){
				$('img.t4').eq(i).attr('src',json[6][3][i].src);
			}
		...
		function callbackl(json){
			for(var i=0;i<json[0].length;i++){
				...
				$('img.d7').eq(i).attr('src',json[5][i].src);
			}
		}
		...
		function callbackp(json){
			...
			$.each(json[$(pro_nav).index()],function(i,ele){
				...
				$('#nav .pro_nav .left ul li').on({
					'mouseenter':function(){
						...
						$('#nav .pro_nav .left .left_img img').attr('src',json[$(pro_nav).index()][$(this).index()].src);
			...
			$.each(json[$(pro_nav).index()],function(i,ele){
				...
				$('#nav .pro_nav .right ul li').on({
					'mouseenter':function(){
						console.log(json[$(pro_nav).index()][$(this).index()].src);
						$('#nav .pro_nav .right .right_img img').attr('src',json[$(pro_nav).index()][$(this).index()].src);
						$(this).removeClass('add').toggleClass('add');
					},
					'mouseleave':function(){
						$(this).removeClass('add');
		...
		$.each(json[$(gift_nav).index()],function(i,ele){
				...
				$('#nav .gift_nav .left ul li').on({
					'mouseenter':function(){
						...
						$('#nav .gift_nav .left .left_img img').attr('src',json[$(gift_nav).index()][$(this).index()].src);
						$(this).removeClass('add').toggleClass('add');
					},
					'mouseleave':function(){
						$(this).removeClass('add');
			...
			$.each(json[$(gift_nav).index()],function(i,ele){
				...
				$('#nav .gift_nav .right ul li').on({
					'mouseenter':function(){
						// console.log(json[$(gift_nav).index()][$(this).index()].src);
						$('#nav .gift_nav .right .right_img img').attr('src',json[$(gift_nav).index()][$(this).index()].src);
						$(this).removeClass('add').toggleClass('add');
					},
					'mouseleave':function(){
						$(this).removeClass('add');
				...
				if (_cookie) {
					 ...
					 for(var x=0;x<_cookie.length;x++){
				  		...
				  		if(_cookie[x].userName == name){
				  			...
				  			var img = $('<img/>').attr('src',_cookie[x].img);
